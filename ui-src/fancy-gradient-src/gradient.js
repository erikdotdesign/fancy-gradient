...
import { MiniGl, normalizeColor } from './webgl';
...
export class Gradient {
  ...
  changeGradientColors(colors) {
    ...
    this.inputColors = colors;
    this.initGradientColors(); // converts hex to normalized vec3

    // Update uniforms for existing material
    this.mesh.material.uniforms.u_baseColor.value = this.sectionColors[0];
    for (let i = 0; i < this.sectionColors.length - 1; i++) {
      const layer = this.mesh.material.uniforms.u_waveLayers.value[i];
  ...
  toggleDarkenTop() {
    ...
    if (!uniform) return;
    uniform.value = uniform.value === 1 ? 0 : 1;
    this.minigl.render(); // re-render after change
  }
}
